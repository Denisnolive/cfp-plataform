...
- [ ] 3.1 Criar o serviço `CfpService` em `apps/frontend` consumindo `SpeakerDTO` para realizar a requisição HTTP `POST /api/cfp` e verificar tipagem estrita
- [ ] 3.2 Desenvolver o componente standalone `CfpSubmissionComponent` utilizando Angular Signals (`signal`, `computed`) para gerenciar todo o estado dos campos, validação reativa e controle do fluxo de submissão
- [ ] 3.3 Implementar conformidade de acessibilidade WAI-ARIA no template do `CfpSubmissionComponent` (`aria-required`, `aria-invalid`, `aria-describedby`, `role="alert"`, `aria-live`) e vincular o bloqueio do botão de envio ao computed Signal de validade (`[disabled]` e `[attr.aria-disabled]`)
- [ ] 3.4 Configurar rota ou navegação para o `CfpSubmissionComponent` no frontend e verificar compilação com `nx build frontend`
- [ ] 3.5 Criar testes unitários com Jest para o `CfpSubmissionComponent`, validando explicitamente o estado inicial limpo dos Signals e o bloqueio do botão de envio no carregamento, e verificar execução com `nx test frontend`
